...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    var deliveryPropio = await sql`SELECT COUNT(DISTINCT session) as total FROM chats WHERE ts > NOW() - INTERVAL '7 days' AND (user_msg ILIKE '%pedir.burgerjazz%' OR user_msg ILIKE '%pedidos.burgerjazz%' OR user_msg ILIKE '%delivery propio%' OR user_msg ILIKE '%vuestra web%' OR user_msg ILIKE '%vuestra pagina%' OR bot_msg ILIKE '%pedir.burgerjazz.com%' OR bot_msg ILIKE '%pedidos.burgerjazz.com%')`;
